...
// 条件付きimportを関数化
const getLivereload = async () => {
  if (process.env.LIVERELOAD === 'true') {
    const { default: connect } = await import('gulp-connect')
    return connect.reload
  }
  return undefined
}

const livereload = await getLivereload()
const serverConfig = { host: '0.0.0.0', port: 5252, livereload }

// タスクのインポート
import tasks from './gulp.d/tasks/index.js'
...
const cleanTask = createTask({
  ...
  call: tasks.remove(['build', 'public']),
})

const lintCssTask = createTask({
  name: 'lint:css',
  desc: 'Lint the CSS source files using stylelint (standard config)',
  call: tasks.lintCss(glob.css),
})

const lintJsTask = createTask({
  name: 'lint:js',
  desc: 'Lint the JavaScript source files using eslint (JavaScript Standard Style)',
  call: tasks.lintJs(glob.js),
})
...
const buildPreviewPagesTask = createTask({
  ...
  call: tasks.buildPreviewPages(srcDir, previewSrcDir, previewDestDir, livereload),
})
...
const previewServeTask = createTask({
  name: 'preview:serve',
  call: tasks.serve(previewDestDir, serverConfig, () => watch(glob.all, previewBuildTask)),
})
